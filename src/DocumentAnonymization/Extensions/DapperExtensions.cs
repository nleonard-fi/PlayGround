...
namespace DocumentAnonymization.Extensions;


public static class DapperExtensions
{
    //Ref: https://hyr.mn/dapper-and-polly/ and https://hyr.mn/Polly-wait-and-retry

    /// <summary>
    /// ExecuteAsync using a Wait and Retry with Jittered Back-off policy.
    /// https://github.com/Polly-Contrib/Polly.Contrib.WaitAndRetry#wait-and-retry-with-jittered-back-off
    /// </summary>
    /// <param name="conn">The connection to query on.</param>
    ...
    /// <returns>The number of rows affected if SET NOCOUNT ON is not set on the query/sproc; otherwise -1.</returns>
    public static async Task<int> ExecuteAsyncWithRetry(this IDbConnection conn, string sql, object param = null!,
        IDbTransaction transaction = null!, int? commandTimeout = null, CommandType? commandType = null, int maxRetries = 10)
    {
        ...
        var retryPolicy = Policy
            ...
            .WaitAndRetryAsync(delay,
                (exception, timeSpan, retryCnt, context) =>
                {
                    Log.Warning(exception,
                        "{Method}: Error talking to Database, will retry after {TimeSpan}. Retry attempt {RetryCount}/{MaxRetries}",
                        nameof(ExecuteAsyncWithRetry), timeSpan, retryCnt, maxRetries);
                });
...
// In addition, it includes SQL Error 11001 (Connection failed).
...
public static class SqlServerTransientExceptionDetector
{
    public static bool ShouldRetryOn(SqlException ex)
    {
        if (ex is SqlException sqlException)
        {
            ...
            {
                Log.Warning("SqlException error #: {ErrorNumber}", err.Number);
                switch (err.Number)
                {
    ...
    public static bool ShouldRetryOn(Win32Exception ex)
    {
        switch (ex.NativeErrorCode)
        {
            // Timeout expired
            case 0x102:
                Log.Warning("Win32Exception Error Code: 0x102 Timeout");
                return true;
            // Semaphore timeout expired
            case 0x121:
                Log.Warning("Win32Exception Error Code: 0x121 Semaphore Timeout");
                return true;
            default:
                return false;
        }
